...
/*   Updated: 2020/01/20 13:49:56 by pcoureau         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_dotx_ii(char *tampozero, t_ftprintf *mas, int len, char *tampostr)
{
...
char	*ft_dotx(t_ftprintf *mas, int len, char *tampostr, unsigned long xnum)
{
	char	*tampozero;
	...
	if (mas->dot_activ)
	{
		...
		if (mas->dot > 0 && mas->dot > len)
		{
			...
			if (ft_dotx_ii(tampozero, mas, len, tampostr) < 0)
				return (NULL);
...
void	ft_starx_ii(t_ftprintf *mas, int len, char *tampospace)
{
	if (mas->zero_activ)
...
void	*ft_starx(t_ftprintf *mas, int len, char *tampostr)
{
	char	*tampospace;
	...
	if (mas->star_activ && mas->star != 0 && mas->star > len)
	{
		if (mas->star > len)
		{
			...
			ft_starx_ii(mas, len, tampospace);
		}
...
void	*ft_convx(va_list av, t_ftprintf *mas)
{
	...
	if (mas->dot_activ)
		if ((tampostr = ft_dotx(mas, len, tampostr, xnum)) == 0)
			return (NULL);
	len = ft_strlen(tampostr);
	if (mas->star < 0)
	{
	...
	return (ft_starx(mas, len, tampostr));
}
